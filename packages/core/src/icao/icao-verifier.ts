...
export interface IcaoVerifierOptions {
  ...
  checkOcsp?: (cert: CertificateInfo) => Promise<{ status: string; error?: string }>;
...
export class IcaoVerifier {
  private options: IcaoVerifierOptions;
   ...
   */
  async verify(rawQrData: Uint8Array): Promise<QrVerificationResult> {
    this.steps = [];

    try {
      // ── Paso 1: Decodificar VDS ──────────────────────────
      const parsed = await this.executeStep(
        'Decodificar QR (ICAO 9303)',
        () => VdsParser.parse(rawQrData),
      );

      // ── Paso 2: Extraer referencia del certificado ───────
      const certRef = await this.executeStep(
        'Extraer referencia del certificado',
        () => {
          const ref = parsed.header.certificateReference;
          if (!ref) throw new Error('No certificate reference found in VDS header');
          return ref;
        },
      );

      // ── Paso 3: Descargar certificado de PKI ─────────────
      const certificate = await this.executeStep(
        'Descargar certificado de PKI',
        async () => {
      ...
      await this.executeStep(
        'Verificar estado OCSP del certificado',
        async () => {
          let ocspResult;
      ...
      await this.executeStep(
        ...
        async () => {
          ...
          if (this.options.verifySignature) {
            isValid = await this.options.verifySignature(
              certificate.publicKeyDer,
              parsed.signedData,
              parsed.signature.signatureBytes,
              parsed.signature.algorithm,
            );
          } else {
            isValid = await EcdsaVerifier.verify(
              certificate.publicKeyDer,
              parsed.signedData,
              parsed.signature.signatureBytes,
              parsed.signature.algorithm,
      ...
      const expiryInfo = await this.executeStep(
        'Comprobar caducidad del QR (5 min)',
        () => {
          ...
          if (elapsed > QR_EXPIRY_MS) {
            throw new Error(
              `QR code expired: generated ${Math.round(elapsed / 1000)}s ago ` +
   ...
   */
  private async executeStep<T>(name: string, fn: () => T | Promise<T>): Promise<T> {
   ...
   */
  private classifyError(message: string): QrVerificationResult['errorCode'] {
    const lower = message.toLowerCase();
    if (lower.includes('magic')) return 'INVALID_MAGIC';
    if (lower.includes('version')) return 'UNSUPPORTED_VERSION';
    if (lower.includes('tlv')) return 'TLV_PARSE_ERROR';
    if (lower.includes('c40')) return 'C40_DECODE_ERROR';
    if (lower.includes('certificate') && lower.includes('not found')) return 'CERTIFICATE_NOT_FOUND';
    if (lower.includes('certificate') && lower.includes('download')) return 'CERTIFICATE_DOWNLOAD_ERROR';
    if (lower.includes('ocsp')) return 'OCSP_ERROR';
    if (lower.includes('revoked')) return 'CERTIFICATE_REVOKED';
    if (lower.includes('signature')) return 'SIGNATURE_INVALID';
    if (lower.includes('expired') || lower.includes('caducad')) return 'QR_EXPIRED';
