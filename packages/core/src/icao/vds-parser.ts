...
import type { VdsHeader, VdsMessage, VdsSignature, VdsParsedData, DniData } from '../types/index.js';
...
export class VdsParser {
  ...
  static parse(rawData: Uint8Array): VdsParsedData {
    ...
    if (magic !== ICAO_CONSTANTS.VDS_MAGIC) {
      throw new Error(
        `Invalid VDS magic byte: expected 0x${ICAO_CONSTANTS.VDS_MAGIC.toString(16)}, ` +
   ...
   */
  private static decodeCertificateReference(data: Uint8Array): string {
   ...
   */
  private static decodeBcdDate(bcd: Uint8Array): string {
  ...
  private static decodeTlvData(
    tlvNodes: Array<{ tag: number; length: number; value: Uint8Array }>,
    ...
    for (const node of tlvNodes) {
      const valueStr = () => {
        try {
          ...
          if (c40.length > 0 && /^[\x20-\x7E]+$/.test(c40)) return c40;
        } catch { /* fallback */ }
   ...
   */
  private static parseIcaoDate(value: Uint8Array): string {
   ...
   */
  private static detectSignatureLength(data: Uint8Array, messageStart: number): number {
   ...
   */
  private static detectSignatureAlgorithm(length: number): string {
    if (length <= 72) return ICAO_CONSTANTS.SIGNATURE_ALGORITHMS.ECDSA_SHA256;
    if (length <= 104) return ICAO_CONSTANTS.SIGNATURE_ALGORITHMS.ECDSA_SHA384;
   ...
   */
  private static getDocumentType(category: number): string {
