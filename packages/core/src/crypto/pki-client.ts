...
export class PkiClient {
   ...
   */
  static async downloadCertificate(certificateReference: string): Promise<CertificateInfo> {
    ...
    try {
      ...
      if (error instanceof TypeError && (error as Error).message.includes('fetch')) {
        throw new Error(`Network error downloading certificate: ${(error as Error).message}`);
      }
      throw error;
    }
  }
   ...
   */
  static async listCertificates(): Promise<string[]> {
    try {
      ...
      try {
        ...
        if (json.certificates) return json.certificates;
      } catch {
   ...
   */
  private static parseX509Basic(der: Uint8Array): {
    subject: string;
    issuer: string;
    notBefore: Date;
    notAfter: Date;
    ...
    const readTL = (data: Uint8Array, offset: number): { tag: number; length: number; headerSize: number } => {
      let tag = data[offset++];
      // Tag multi-byte
      if ((tag & 0x1F) === 0x1F) {
    ...
    if (this.findBytes(der, new Uint8Array(p384Oid)) >= 0) curve = 'P-384';
    if (this.findBytes(der, new Uint8Array(p521Oid)) >= 0) curve = 'P-521';
   ...
   */
  private static findBytes(haystack: Uint8Array, needle: Uint8Array): number {
   ...
   */
  private static isPem(data: Uint8Array): boolean {
   ...
   */
  private static pemToDer(pem: Uint8Array): Uint8Array {
