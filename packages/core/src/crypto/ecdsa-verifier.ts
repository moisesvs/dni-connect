...
export class EcdsaVerifier {
  ...
  ): { hashAlgorithm: string; namedCurve: string } {
   ...
   */
  private static derToRawSignature(signature: Uint8Array, namedCurve: string): Uint8Array {
    ...
    try {
      ...
      // Leer INTEGER r
      if (signature[pos] !== 0x02) throw new Error('Expected INTEGER tag for r');
      pos++;
      const rLength = signature[pos++];
      let rBytes = signature.slice(pos, pos + rLength) as Uint8Array<ArrayBuffer>;
      pos += rLength;

      // Leer INTEGER s
      if (signature[pos] !== 0x02) throw new Error('Expected INTEGER tag for s');
      pos++;
      const sLength = signature[pos++];
      let sBytes = signature.slice(pos, pos + sLength) as Uint8Array<ArrayBuffer>;

      // Normalizar: quitar leading zeros y ajustar tamaño
      rBytes = this.normalizeComponent(rBytes, componentSize) as unknown as Uint8Array<ArrayBuffer>;
      sBytes = this.normalizeComponent(sBytes, componentSize) as unknown as Uint8Array<ArrayBuffer>;
   ...
   */
  private static normalizeComponent(component: Uint8Array, targetSize: number): Uint8Array {
    // Eliminar leading zeros
    let start = 0;
    while (start < component.length - 1 && component[start] === 0) {
      start++;
    }
    const trimmed = component.slice(start) as Uint8Array;
    ...
    if (trimmed.length > targetSize) {
      ...
      result.set(trimmed.slice(trimmed.length - targetSize) as Uint8Array);
    } else {
      // Pad con zeros
   ...
   */
  private static getComponentSize(namedCurve: string): number {
    switch (namedCurve) {
   ...
   */
  static async hash(algorithm: string, data: Uint8Array): Promise<Uint8Array> {
