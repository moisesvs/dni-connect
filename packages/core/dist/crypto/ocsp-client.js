...
export class OcspClient {
     ...
     */
    static async check(certificate) {
        try {
     ...
     * }
     */
    static buildOcspRequest(certificate) {
        ...
        const serialNumber = this.derEncodeInteger(this.hexToBytes(certificate.reference.replace(/[^0-9A-Fa-f]/g, '').padStart(2, '0')));
        // CertID
        const certId = this.derSequence(this.concat(sha1AlgorithmIdentifier, this.derOctetString(issuerNameHash), this.derOctetString(issuerKeyHash), serialNumber));
        // Request
     ...
     */
    static findCertStatus(data, startPos) {
...
//# sourceMappingURL=ocsp-client.js.map