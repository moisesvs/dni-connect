...
syncRouter.post('/', async (req, res, next) => {
    try {
        ...
        if (parsed.data.dniData.facialImageBase64) {
            await storeImageInGCS(verificationId, parsed.data.dniData.facialImageBase64);
        }
 ...
 */
syncRouter.get('/history', async (req, res, next) => {
    try {
 ...
 */
async function storeInFirestore(payload) {
    // En producción:
 ...
 */
async function storeImageInGCS(verificationId, base64Image) {
    // En producción:
